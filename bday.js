...
const BD_Name = document.querySelector("#Name");
const FormDiv = document.querySelector("#formdiv");
const AfnanDiv = document.querySelector("#WishAfnan");
...
let WishWords = [
    ...
    "Sending happy birthday wishes to a great person, I hope your day is filled with joy, happiness, fun, and new avenues. Happy birthdays!"
];
let randomWish = WishWords[Math.floor(Math.random()*WishWords.length)];
let BDaySong = new Audio("./Media/Happy Birthday To You Ji.mp3");
window.addEventListener("load",function (){
  loader.className += " hidden";
  FormDiv.classList.remove("clear");
});
Form.addEventListener("submit", (e)=>{
    e.preventDefault();
    let checker = BD_Name.value;
    let afnanList = ["afnan", "mohammad afnan", "mohammad afnan pasha", "afnan pasha"];
    ...
    if(afnanList.includes(checker.toLowerCase())){
      FormDiv.classList.add("clear");
        ...
        ImgBox.classList.add("afnan");
        WishText.innerHTML = randomWish;
        BDaySong.play();
    }
    if(razaList.includes(checker.toLowerCase())){
      ...
      window.addEventListener("load",function (){
        ...
        FormDiv.classList.add("clear");
        AfnanDiv.classList.add("wish");
        WishName.innerText = "Raza";
        ImgBox.classList.add("raza");
        WishText.innerHTML = randomWish;
        BDaySong.play();
    }
})
